...
*** Overview
At its most basic, we read data from a [[program-backend][plain text file]] containing Lisp plists, store it in a [[program-data-structures][hash table]], and display it as a [[elisp:(find-library "tabulated-list-mode")][=tabulated-list-mode=]] buffer. When the file is changed, whether by the program or the user, we update the hash table and the buffer.
