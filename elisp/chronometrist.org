...
Issues
1. Emacs may freeze at the prompt for file-local variable values; C-g will quit the prompt, and permanently marking them as safe will make the freezing stop.
** Commentary
...
# eval: (nameless-mode)
# eval: (add-hook 'after-save-hook (lambda () (start-process-shell-command "sed-tangle" (generate-new-buffer-name "sed-tangle") "sed -n -e '/#+BEGIN_SRC emacs-lisp$/,/#+END_SRC$/{//!p;};/#+END_SRC/i\\ ' chronometrist.org > chronometrist.el")))
# End:
