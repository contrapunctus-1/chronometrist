...
1. [-] (tag|key-value)-combination-prompt - select combinations of (tags|key-values)
   * commands
...
2. [ ] (tag|key|value)-multiselect-prompt - select individual (tags|keys|values)
   * commands
...
3. [-] unified-prompt - tag-key-value combinations, everything in one prompt
   * commands
     + 0-9     - use combination (and exit)
     + C-u 0-9 - edit combination (then exit)
     + s       - skip (exit)
