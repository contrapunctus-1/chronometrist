...
8. [ ] Enhanced tag/key-value prompt - before asking for tags/key-values, if the last occurence of task had tags/key-values, ask if they should be reused. y - yes, n - no (continue to usual prompts).

** chronometrist-report
1. [ ] Jump to beginning/end of data (keys B/F)
2. [ ] Expandable items - show tag-combination-based breakdown
