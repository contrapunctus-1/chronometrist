   ...
   7. [-] Time targets/goals
      ...
      * [ ] Notify user when
        ...
        + [ ] I wonder if there can be a more flexible way to define alerts. User specifies name, time to run and whether to repeat (passed to ~run-at-time~), alert text (and other parameters for ~alert~)
          ...
          An element can alternatively be a lambda/function returning a list of this form, which will be called with the task name and the goal.
          We map over that list, start the timers, and append their values to a global list. To stop the timers, we map over that list.
