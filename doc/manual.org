...
**** Other strategies
1. Deferring (tag and key-value) history generation. Instead of generating the history for all tasks whenever the file changes, generate them when they are prompted for, and only for the task that is being prompted. (see branch =delay-history=)
